...
export default function ProjectPage() {
  ...
  const { data: userRole, isLoading: isRoleLoading } = trpc.projectMembers.getMyRole.useQuery({ projectId });

  // Redirect if user doesn't have access
  useEffect(() => {
    if (!isLoading && !isRoleLoading && !project) {
      router.push('/dashboard');
    }
    if (!isLoading && !isRoleLoading && project && userRole === null) {
      router.push('/dashboard');
    }
  }, [project, userRole, isLoading, isRoleLoading, router]);
    ...
    <ProtectedRoute>
      <DashboardLayout fullWidth={true}>
        <div className="flex h-[calc(100vh-4rem)]">
          ...
          <aside className="w-56 bg-background-secondary border-r border-border-subtle overflow-y-auto flex-shrink-0">
            <div className="p-4">
              {/* Project Header */}
