  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {canEdit && (
          <div className="flex gap-2 flex-wrap">
            ...
            >
              🔗 Create from Crew/Equipment/Locations
            </button>
            <button
      ...
      {canEdit && selectedItems.size > 0 && (
        <div className="bg-background-tertiary border border-border-default rounded-lg p-4 flex items-center justify-between">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border-default">
              {filteredCategories.map((category: any) => {
                ...
                const categoryEst = categoryItems.reduce((sum, item) => sum + (item.estimatedAmount || 0), 0);
                const categoryAct = categoryItems.reduce((sum, item) => sum + (item.actualAmount || 0), 0);
      ...
      {showCreateFromModal && (
        <CreateFromModal
          ...
          onCreateFromCrew={(ids) => createFromCrew.mutate({ projectId, crewMemberIds: ids })}
          onCreateFromEquipment={(ids) => createFromEquipment.mutate({ projectId, equipmentIds: ids })}
          onCreateFromLocations={(ids) => createFromLocations.mutate({ projectId, locationIds: ids })}
          onCreateFromCast={(ids) => createFromCast.mutate({ projectId, castMemberIds: ids })}
        />
      )}
      {showTemplateModal && (
...
function BudgetItemRow({
  ...
  // Calculate estimated amount from unitRate * quantity if unitRate exists
